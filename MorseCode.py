...
def Build_Character_Encoder():
    ...
    for n in range(0, len(CodeList1)):
        ce1[CharList1[n]]=CodeList1[n]
    for n in range(0, len(CodeList2)):
        ce2[CharList2[n]]=CodeList2[n]
    for n in range(0, len(CharList3)):
        ce3[CharList3[n]]='.-.-.-'
    
  
    Character_Encoder.update(ce1)
    Character_Encoder.update(ce2)
    Character_Encoder.update(ce3)
...
def Build_Character_Decoder():
    ...
    for n in range(0, len(CodeList1)):
        cd1[CodeList1[n]]=CharList1[n]
    for n in range(0, len(CodeList2)):
        cd2[CodeList2[n]]=CharList2[n]

    cd3['.-.-.-']='#'

    Character_Decoder.update(cd1)
    Character_Decoder.update(cd2)
    Character_Decoder.update(cd3)
...
def Word_Encoder(Word, Character_Encoder=None):
    ...
    # add your code from here
    Wordlc_to_list = list(Word.lower())
    Word_in_Morse_Code_list = []
    Word_in_Morse_Code=''
    
    for n in range(0, len(Wordlc_to_list)):
        Word_in_Morse_Code_list.append(Character_Encoder[Wordlc_to_list[n]])
        
    for n in range(0, len(Word_in_Morse_Code_list)-1):
        Word_in_Morse_Code += Word_in_Morse_Code_list[n] + ' '
        
    Word_in_Morse_Code += Word_in_Morse_Code_list[len(Word_in_Morse_Code_list)-1] 
...
def Word_Decoder(Word_in_Morse_Code, Character_Decoder=None):
    ...
    Word_list =[]
    Word=''
    Word_in_Morse_Code_list = Word_in_Morse_Code.split(' ')
    
    for n in range(0, len(Word_in_Morse_Code_list)):
        Word_list.append(Character_Decoder[Word_in_Morse_Code_list[n]])
...
def Message_Encoder(Message, Character_Encoder=None):
    ...
    Message_in_Morse_Code_list =[]
    Message_in_Morse_Code=''
    
    for n in Message_list:
        Message_in_Morse_Code_list.append(Word_Encoder(n))
        
    for n in range(0, len(Message_in_Morse_Code_list)-1):
        Message_in_Morse_Code += Message_in_Morse_Code_list[n] + ','
        
    Message_in_Morse_Code = Message_in_Morse_Code + Message_in_Morse_Code_list[len(Message_in_Morse_Code_list)-1] 
...
def Message_Decoder(Message_in_Morse_Code, Character_Decoder=None):
    ...
    Message_list =[]
    Message=''
    Message_in_Morse_Code_list = Message_in_Morse_Code.split(',')
    
    
    for n in range(0, len(Message_in_Morse_Code_list)):
        Message_list.append(Word_Decoder(Message_in_Morse_Code_list[n]))
        
    for n in range(len(Message_list)-1):
        Message += Message_list[n] + ' '
